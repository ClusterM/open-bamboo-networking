...
static int test_cert_id_constant()
{
    ...
    const std::string env     = obn::signing::maybe_sign(payload);
    CHECK(envelope_field(env, "cert_id") == "a4e8faaa1a38e3650a0ea590d192383fCN=GLOF3813734089.bambulab.com");
    return 0;
}
...
int main()
{
    ...
    FILE* f = fopen(pem_path.c_str(), "w");
    if (!f) { std::cerr << "fopen pem failed\n"; EVP_PKEY_free(g_test_key); return 1; }
    ...
    // Run all tests (same list as before).
    int rc = 0;
