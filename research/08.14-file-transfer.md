...
This subsystem is initialized right after `bambu_networking` loads, via `InitFTModule(networking_module)`, and resolves its symbols from the same module ([FileTransferUtils.hpp](https://github.com/bambulab/BambuStudio/blob/12f17b06f4f537f9c03162d08bb70cf733c42839/src/slic3r/Utils/FileTransferUtils.hpp), `FileTransferUtils.cpp`):

Source: [FileTransferUtils.hpp:71-95](https://github.com/bambulab/BambuStudio/blob/12f17b06f4f537f9c03162d08bb70cf733c42839/src/slic3r/Utils/FileTransferUtils.hpp#L71-L95)
...
Source: [FileTransferUtils.hpp:27-40](https://github.com/bambulab/BambuStudio/blob/12f17b06f4f537f9c03162d08bb70cf733c42839/src/slic3r/Utils/FileTransferUtils.hpp#L27-L40)
...
#### 8.14.1. Stock wire transport (TLS :6000)

Stock `libbambu_networking.so` serves LAN `ft_*` jobs over the very same `:6000` wire as Device → Files — TLS, login + `mtype` 12291 setup, then framed CTRL JSON (`mtype` 12289). The framing, handshake, chunked-upload flow (`chunk_size` in KiB, full-duplex progress), storage labels and result codes are documented once in the **canonical `:6000` protocol reference ([§6.4](06.04-port-6000.md))**; this section only maps the `ft_*` ABI `cmd_type`s onto that wire.

**Send to Printer model upload is `:6000`, not FTPS.** The `.3mf` body goes over **`ft_*` / TLS :6000** (§8.14.2). Stock does touch FTPS :990 first in some flows, but only for a **tiny `verify_job` write probe** (§8.14.3) — then `QUIT`, session closed, never reused. Our earlier tcpdump of *cache-only* Send to Printer saw zero :990 packets because that UI path may skip the probe when the target storage is already known (`emmc` / `udisk` via `cmd_type=7`).
...
| `4` (mem preview) | `0x0004` (`FILE_DOWNLOAD`) | Wire: `{"cmdtype":4,"sequence":N,"req":{"path":"mem:/26","offset":0}}` — **no** `is_mem_file` on wire | `ft_job_result.bin` = JPEG; `json` = final `reply` (`file_md5`, …). See §8.14.4 |
| `5` (upload) | `0x0005` | **Chunked** upload — see §8.14.2 (not the legacy one-shot `{path,file,size,md5}` shape) | `resp_ec=0` on success; progress via `msg_cb({"progress":N})` while sending |
...
#### 8.14.2. P2S `FILE_UPLOAD` (`cmdtype` 5) — chunked pipeline (May 2026)
...
All JSON bodies are wrapped for the wire as `{"mtype":12289,…}` (§9.5.1.1) and sent inside a 16-byte frame header (`magic=0x0102013f`). Chunk payloads append `\n\n` + raw bytes **inside the same frame** as the chunk JSON.
...
Any client on a reused tunnel must drain or filter stale JSON before upload init and must match replies on **`cmdtype` + `sequence`**, not bare `result:0`. 
...
MQTT capability flags (`print.fun` / `print.fun2`) govern which tabs Studio **shows** (e.g. model internal storage); they are orthogonal to the upload wire format. After upload, print-start uses `project_file` with a `url` scheme ([§12.1](12.01-project-file.md) **URL schemes**) — typically `brtc://emmc/<name>` for cache-resident Send-to-Printer jobs, not a repeat of the `:6000` `storage` label.
...
Not listed in Studio's `PrinterFileSystem.h` success enum (§9.5.3). On P2S we consistently saw `-9203` when violating the pipeline rules (per-chunk recv, or post-send SSL poll mid-pipeline).

##### Progress reporting (`msg_cb`)

During pipelined send, stock reports upload progress from **bytes written** (`offset / total`), not from per-chunk wire ACKs (there are none until the final reply). 
#### 8.14.3. `start_send_gcode_to_sdcard` (FTPS upload, no print)
...
#### 8.14.4. P2S `FILE_DOWNLOAD` (`cmdtype` 4) — mem preview (Printer Preview)

Reverse-engineered on **P2S** (`02.07.00.99` firmware, May 2026) with wire logging and [`tools/bambu6000_repl.py`](../tools/bambu6000_repl.py). This is **not** Send to Printer and **not** Device → Files (those use `libBambuSource.so` for file-browser CTRL, or `ft_*` upload for cache).
...
Sources: [`FileTransferObject.cpp`](https://github.com/bambulab/BambuStudio/blob/12f17b06f4f537f9c03162d08bb70cf733c42839/src/slic3r/Utils/FileTransferObject.cpp), [`MediaPlayCtrl.cpp`](https://github.com/bambulab/BambuStudio/blob/12f17b06f4f537f9c03162d08bb70cf733c42839/src/slic3r/GUI/MediaPlayCtrl.cpp) ([MediaPlayCtrl.cpp:860](https://github.com/bambulab/BambuStudio/blob/12f17b06f4f537f9c03162d08bb70cf733c42839/src/slic3r/GUI/MediaPlayCtrl.cpp#L860)).
...
Same framing as §8.14.2: `{"mtype":12289,…}` inside 16-byte header (`magic=0x0102013f`), optional `\n\n` + binary in the same frame body.
...
Stock `libbambu_networking.so` (~31 MB) links OpenSSL **statically and stripped** — Frida often finds **no** exported `SSL_write`/`SSL_read`, so plaintext wire does not appear in `/tmp/ft_wire.log`. ABI hooks still log `ft_job_create` / `result_cb`. Options: use a non-stripped plugin build for SSL hooks, `SSLKEYLOGFILE` + Wireshark, or [`tools/bambu6000_repl.py`](../tools/bambu6000_repl.py) `/download`. See [`tools/frida_ft_attach.sh`](../tools/frida_ft_attach.sh).
...
```bash
DEV_ID=… DEV_IP=… ACCESS_CODE=… \
  tools/probe_remote_naming.sh
# optional: ABI=02.05.03  ACTION=local_print  (last table row)
# compare alternate plugin: PLUGIN_PATH=build/libbambu_networking.so
```

Hermetic naming logic (no printer): [`tests/remote_name_test.cpp`](../tests/remote_name_test.cpp) — `ctest -R remote_name` with `-DOBN_BUILD_TESTS=ON` (CMake test flag).
...
Used by **`start_local_print`**, **`start_local_print_with_record`**, and the cloud FTPS leg — **not** by `start_send_gcode_to_sdcard`. Logic (stock behaviour on N7, May 2026):
...
Unit tests: [`tests/remote_name_test.cpp`](../tests/remote_name_test.cpp) (`ctest -R remote_name` when built with `-DOBN_BUILD_TESTS=ON` (CMake test flag)).
...
On **P2S / brtc** hardware, Send to Printer model bytes use **`ft_*` / `:6000`** (§8.14.2), not this ABI.
...
| 2025-10 | `76e45bde2` *print with emmc* + `662b7fdac` *change send mode* | **`ft_*` `:6000` cache upload** when **`MachineObject::is_support_brtc`** (`print.fun` bit 31); SendToPrinter prefers TCP/TUTK tunnel + `CreateUploadFileJob`; FTPS `SendJob` path kept as **else** fallback — see [§11.1](11.01-studio-orchestration.md) / [§12.5](12.03-push-status-capabilities.md) **Printer capability flags** |
...
**Do not conflate with Device → Files.** Stock file browsing on P2S stays on `:6000` only. FTPS :990 during Device → Files is a separate topic (§9.5.1); Send to Printer uses `ft_*` in `libbambu_networking.so` (§8.14).
