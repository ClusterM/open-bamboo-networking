...
— on the cloud API host; see [Cloud HTTP transport](13-http-transport.md) for base URL, headers and the common response envelope that apply here and to every other HTTP endpoint the plugin touches. Preset IDs are prefixed by type (observed in live responses):
...
#### 8.9.2. `values_map` keys the loader expects
...
`GUI_App::start_sync_user_preset()` drives the whole thing on a worker thread ([GUI_App.cpp](https://github.com/bambulab/BambuStudio/blob/12f17b06f4f537f9c03162d08bb70cf733c42839/src/slic3r/GUI/GUI_App.cpp)):
