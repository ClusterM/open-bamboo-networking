...
| `ftp_file_md5` | `std::string` | — | | **Unused by Studio** — never assigned in `PrintJob` (same as `ftp_file`). Stock LAN MQTT ignores it (`md5:"from_sd_card"` even with a hex sentinel). Stock cloud `PATCH profile_print_3mf[].md5` is the plugin's own uppercase-hex MD5 of `filename`, not this field ([§8.8.3](#883-bambu_network_start_local_print_with_record)). Intended purpose unknown; the name suggests a precomputed FTPS-body hash, but no capture shows stock reading it. |
...
| `task_timelapse_use_internal` | `bool` | — | **`02.05.03`** | Timelapse to internal storage → MQTT `cfg` bit `0x4`. Absent before this series ⇒ stock can never set that bit (but bit `0x1` is still reachable via `task_ext_change_assist`) |
...
| `task_ext_change_assist` | `bool` | — | | External-spool change assist. No dedicated key anywhere: it is **`cfg` bit 0** in both LAN `project_file` and `POST /my/task` ([§8.8.8](#888-post-mytask-body-which-keys-are-optional)) |
...
| `svc_context` | `std::string` | — | **`02.07.01`** | Optional MakerWorld / model metadata (`BBL_SVC_CONTEXT_TAG`); Studio copies when present (`PrintJob.cpp`). → cloud `svcContext` **only** (see below) |
| `slicer_uid` | `std::string` | — | **`02.08.01`** | Slicer instance uid. Header-only in Studio `02.08.02.60` (no write site), but the plugin **does** forward it: → LAN MQTT `slicer_uid` **only** (see below) |
| `queue_plate_id` | `std::string` | — | **`02.08.02`** | Print Queue plate id, added alongside the unused `PrintQueue*` structs ([§8.16.9](08.16-errors.md#8169-reserved-print-queue-36-59)). No Studio write site, but the plugin forwards it: → cloud `plateId` as a **number** (see below) |

**`svc_context` / `slicer_uid` / `queue_plate_id` on the wire.** All three are forwarded by stock `02.08.02.54`, each to exactly one destination and none to both. Measured by running the stock plugin under `tools/plugin_runner` with three parameter sets — one control with the fields unset, one with alphabetic sentinels, one with numeric sentinels — while watching LAN MQTT, cloud MQTT and HTTPS simultaneously (method in [§8.8.7](#887-sentinel-probing-printparams-fields)):

| Field | LAN MQTT `project_file` | `POST /my/task` | Shape | Omitted when |
| --- | --- | --- | --- | --- |
| `slicer_uid` | `slicer_uid` | — | string, verbatim | empty |
| `queue_plate_id` | — | `plateId` | **number**, `strtoll` of the string | empty, unparseable, or `0` |
| `svc_context` | — | `svcContext` | string, verbatim | empty |

Two details only the two-sentinel design could catch. `queue_plate_id` crosses the ABI as a `std::string` but lands on the wire as a JSON *number*, so the alphabetic sentinel silently became `0` and the key vanished — reading that as "not forwarded" is exactly the wrong conclusion, and the numeric set is what disproved it. Its value `9988776655` exceeds 32 bits, matching the `int64_t` plate ids in the `PrintQueue*` structs. Conversely `slicer_uid` and `svc_context` survive both sentinel sets byte-identically, so neither is parsed or normalised.

Both `project_file` capture paths agree: `sdcard_print` (no upload, `md5:"from_sd_card"`, no `url`) and `local_print` (FTPS upload, `url_enc`) carry `slicer_uid` identically. `start_local_print_with_record` publishes no `project_file` at all — the cloud dispatches it — which is why the earlier record-print capture saw none of the three in MQTT.

The stock `project_file` serializer emits keys in alphabetical order (`json_lite` over a `std::map`), placing `slicer_uid` between `sequence_id` and `subtask_id`. Key order carries no meaning for the firmware ([§10.4](10.04-mqtt-signing.md) — only byte-for-byte agreement between the signed string and the envelope matters), so our builder's own ordering stays as is.
...
|---|---|---|---|
| `sequence_id` | string (decimal) | See [§6.2](06.02-mqtt.md#mqtt-sequence_id) | Stock plugin always emits `"20001"` for `project_file` on a fresh process (every ABI `02.05.00`–`02.08.02`). Reusing that value across restarts yields `err_code` 84033544. We seed randomly in 20000–29999 and increment. |
...
| `file` | string | `project_name` / `task_name` via `pick_remote_name` | Basename of the `.3mf`. Stock **ignores `dst_file`** even on `sdcard_print` (a probe with `dst_file="cache/obn-nonexistent-L1.gcode.3mf"` still published `file:"obn-probe-L1.gcode.3mf"`). On `local_print` it matches the uploaded FTPS/brtc name. |
| `url` / `url_enc` | string | `opts.url` | Tells firmware **how to locate** the `.3mf` on `local_print`. **`sdcard_print` omits both keys** (stock `02.05.00`–`02.08.02`) — firmware looks the file up by `file` + `md5:"from_sd_card"`. Cleartext `url` uses one of several schemes (`ftp://`, `brtc://emmc/`, `file://`, presigned `https://…` for cloud); `url_enc` is the same URL RSA-encrypted with the **printer's device-cert public key** using **PKCS#1 v1.5** padding (direct RSA, no AES session-key wrap — the observed ≤~245-byte ciphertext equals the RSA-2048 PKCS#1 v1.5 ceiling of `256−11=245` and rules out OAEP-SHA1, which would cap at 214; independently cross-validated in [issue #47](https://github.com/ClusterM/open-bamboo-networking/issues/47), on-wire confirmation of the padding still pending). Non-Developer-Mode firmware requires `url_enc`; Developer Mode accepts plain `url`. Full scheme semantics: see **URL schemes** below. |
| `md5` | string | literal `"from_sd_card"` | Stock `02.05.00`–`02.08.02` always emit this literal — on `sdcard_print` **and** on FTPS `local_print`, even when `ftp_file_md5` is filled with a hex sentinel (Studio never assigns that field). A real file hash never appeared in any captured LAN `project_file`. Cloud `PATCH` uses a different `md5` — see [§8.8.3](#883-bambu_network_start_local_print_with_record). |
...
| `ams_mapping` | int array | `ams_mapping` (Studio passes a JSON array string `[0,-1,...]`) | Index = filament slot in the 3mf, value = AMS tray id (`-1` = no mapping, `255` = virtual tray). Legacy single-AMS shape. Empty string → `[]` even with `use_ams=true` (stock `02.05.03` / `02.08.02`); there is no `[0]` fallback. |
| `ams_mapping2` | object array | `ams_mapping2` (Studio passes a JSON-array string of `{ams_id, slot_id}` objects) | Newer multi-AMS schema, one entry per filament. `255/255` means "external spool / not via AMS". Required for printers with >1 AMS unit; firmware that supports both fields prefers `ams_mapping2` over `ams_mapping`. **Note:** cloud `POST /my/task` uses a *different* AMS serialization — the plugin re-keys the pairs to camelCase and drops the keys altogether when the mapping is empty, whereas `project_file` always carries both as `[]` ([§8.8.8](#888-post-mytask-body-which-keys-are-optional)). |
...
| `cfg` | string (decimal int) | derived from `task_ext_change_assist` and `task_timelapse_use_internal` | Bitmask the stock plugin builds from `PrintParams` flags that don't have a dedicated MQTT field, and it is **the same bitmask the plugin puts in `POST /my/task`** ([§8.8.8](#888-post-mytask-body-which-keys-are-optional)). **Bit 0 (`0x1`) = `task_ext_change_assist`** (`sdcard_print` with that flag alone published `cfg:"1"`). **Bit 2 (`0x4`) = use internal storage for timelapse**, driven by `task_timelapse_use_internal`. Both set gives `"5"`. Bit 1 (`0x2`) has never been seen and no field this ABI exposes moves it (`try_emmc_print` included). Always emitted as a string, not a number. **Field is present in `project_file` for *every* observed ABI from `02.05.00` upwards** — but on builds older than `02.05.03` (where `PrintParams::task_timelapse_use_internal` does not exist yet) bit `0x4` is unreachable. Cross-ABI confirmation: `02.05.00`/`02.05.01`/`02.05.02` emit `cfg="0"` for a timelapse job; `02.05.03`/`02.06.00`/`02.06.01` emit `cfg="4"` when `task_timelapse_use_internal=true`. **`task_record_timelapse` does NOT influence `cfg`** — only the `timelapse` boolean in the same payload. |
...
###### Cloud-start extras in `project_file`

The **cloud** variant of `project_file` is published by the **cloud**, not the plugin — `start_local_print_with_record` / `start_print` never put this frame on LAN MQTT. Captured on P2S after `/my/task` `mode=lan_file` (stock `02.08.02.54`, 2026-08-16), the dispatched command carries extra keys the LAN builder does not emit:
...
- `design_id` — MakerWorld design id (`0` when the model is not from the store);
- `model_id` — the `US…` id from the project upload;
- `dev_id` — printer serial;
- `plate_idx` — plate index (cloud name; LAN uses `param` instead);
- `bed_temp` — **instead of** `bed_type` (the LAN key is absent);
- `skip_objects` — array, empty in the captures;
- `timestamp` — cloud-side unix time;
- `extrude_cali_manual_mode` — present even when the LAN plugin would have omitted the `-1` default;
- no `file`, no `slicer_uid`.

A trailing `"reason":"success","result":"success"` pair appears on some *published* cloud commands, not only on the printer's ack. The plugin does not need to emit any of this — the cloud does.
...
| `task_use_ams=true` with empty `ams_mapping` / `ams_mapping2` | `use_ams: true`, both arrays stay `[]` — no `[0]` fallback (stock `02.05.03` / `02.08.02`) |
...
5. `PATCH …/user/project/<pid>` — first register, e.g. `{"profile_id":"…","profile_print_3mf":[{"md5":"55705660B4D0360DB2CE5D399D3B4DD0","plate_idx":1,"url":"ftp://….gcode.3mf"}]}`. `md5` is 32 **uppercase** hex of the print-ready `.3mf` (`filename`); stock computes it in-plugin. Not `PrintParams.ftp_file_md5` (Studio leaves that empty). Cloud accepts an all-zero placeholder (`200`) but stock never sends one (`stock_hybrid.mitm` / `stock_hybrid_2.4.0.mitm` / `stock_block_cloud.mitm`, 2026-07).
...
**`amsDetailMapping[]` element** (`amsMapping`/`amsMapping2` are camelCase and *optional*, external-spool sentinel `{amsId:255,slotId:0}` — [issue #48](https://github.com/ClusterM/open-bamboo-networking/issues/48), full key-by-key gating in [§8.8.8](#888-post-mytask-body-which-keys-are-optional)):
...
#### 8.8.7. Sentinel probing `PrintParams` fields

Method used to settle `slicer_uid` / `queue_plate_id` / `svc_context` above; it generalises to any `PrintParams` field whose destination is unclear.

Put an implausible, greppable value in the field, run the stock plugin, and search every wire the job touches for it. **Always use two sentinel sets** — one alphabetic and one all-digits — plus a control set with the fields unset. A field that becomes a JSON number on the wire turns an alphabetic sentinel into `0`, which typically gets dropped, and reads exactly like "not forwarded"; only the numeric set tells those two apart. That is precisely what happened with `queue_plate_id`.

Three capture points, all running at once, because a field may take any one of them:

| Point | Tool | Sees |
|---|---|---|
| LAN MQTT | MQTT client subscribed to `#` on `<printer>:8883` (user `bblp`, password = LAN access code, TLS verification off) | `device/<SN>/request` — what the plugin publishes, and `report` replies |
| HTTPS | `mitmdump -s dump.py -p 8888`, driving the runner with `HTTPS_PROXY` + `CURL_CA_BUNDLE` + `SSL_CERT_FILE` pointed at `~/.mitmproxy/mitmproxy-ca-cert.pem` | `POST /my/task` and the whole record path |
| Cloud MQTT | same client against `<region>.mqtt.bambulab.com:8883`, user `u_<user_id>`, password = access token | what the cloud dispatches back to the printer |

Drive it with [`tools/plugin_runner.sh`](../tools/plugin_runner.sh) `--abi 02.08.02`. Four flags matter and each has a failure mode that looks like something else:

- `--cert-lan-only 0` on a cloud-paired printer. Otherwise the plugin never gets a device certificate, refuses every privileged publish client-side with `rc=-4`, and the action dies at Sending with `-4030` "send msg failed" — which reads like a network or signing problem. (The runner issues `install_device_cert` twice, `lan_only=1` then `lan_only=0`; empirically neither call alone puts `app_cert_install` on the wire.)
- `--client-name BambuStudio`. The cloud answers `POST /my/task` with **403** `"The client does not have access rights to the content."` for any other `X-BBL-Client-Name`, so an honest client name never reaches the body you want to inspect.
- `--user-info @<obn.auth.json>` even for the LAN-only runs — signing material comes from the cloud session.
- `--auto-stop` on anything that can actually start a print.

Two safe dry runs: `--action sdcard_print` with `dst_file` naming a file that isn't on the SD card publishes the full `project_file` and uploads nothing, and reusing `sequence_id` `"20001"` across process restarts makes firmware reject with `mqtt message verify failed` / `err_code` 84033544 — in both cases the plugin's serialization is already on the wire, which is all the probe needs.

Finally diff the *key sets* of the captured bodies rather than eyeballing them: the control run is the baseline, and a key that appears only in the numeric run (`plateId`) is the whole point of the exercise.

#### 8.8.8. `POST /my/task` body: which keys are optional

Full field sweep against stock `02.08.02.54`, probing one `PrintParams` field at a time with the method from [§8.8.7](#887-sentinel-probing-printparams-fields) (four `local_print_with_record` runs under `mitmdump`). The starting point was a key-set diff between a stock body and our own builder, which showed four keys we sent that stock did not, one key we never sent, and one value mismatch.

**Half the body is optional.** Stock omits a key outright rather than sending a default, and the gate is always the individual field — never `task_use_ams`:

| Key | Source field | Emitted when | Notes |
| --- | --- | --- | --- |
| `amsMapping` | `ams_mapping` | string non-empty | Forwarded verbatim. `task_use_ams=true` with an empty string does **not** bring it back; `task_use_ams=false` with a filled string still emits it. Sending `[-1]` as a stand-in over-reports a no-AMS job. |
| `amsMapping2` | `ams_mapping2` | string non-empty | Re-keyed snake→camel **by the plugin itself** (`{"ams_id":2,"slot_id":3}` → `{"amsId":2,"slotId":3}`), which is why Studio's snake_case never reaches the wire and why the endpoint only accepts camelCase ([issue #48](https://github.com/ClusterM/open-bamboo-networking/issues/48)). |
| `nozzleMapping` | `nozzle_mapping` | string non-empty | Verbatim. |
| `extrudeCaliManualMode` | `extruder_cali_manual_mode` | `!= -1` | `-1` is Studio's "not set"; `0` and `1` are both emitted. |
| `designId` | `stl_design_id` | `!= 0` | **Key we were missing entirely.** Number, verbatim. |
| `plateId` | `queue_plate_id` | parses non-zero | [§8.8.1](#881-common-contract). |
| `svcContext` | `svc_context` | non-empty | [§8.8.1](#881-common-contract). |

**`cfg` is the same bitmask as LAN `project_file`,** and it has a second known bit:

| Bit | Value | Driven by |
| --- | --- | --- |
| 0 | `1` | `task_ext_change_assist` |
| 1 | `2` | unknown — no field this ABI exposes moves it, `try_emmc_print` included |
| 2 | `4` | `task_timelapse_use_internal` |

Probed separately in both serializations: a `/my/task` run with `task_ext_change_assist` alone gave `cfg:"5"` alongside internal timelapse and `cfg:"1"` alone, and an `sdcard_print` run gave `"cfg":"1"` in the published `project_file`. `task_record_timelapse` stays out of `cfg` in both — it rides in the separate `timelapse` boolean, and a run with `task_record_timelapse=true` and nothing else set still reported `cfg:"0"`.

**`sequence_id` is not a constant.** It counts POST *attempts* within one `create_task` call: the first is always `20001`, and retries bump it (`20002`, `20003` observed on the 400 and 403 retry bursts), while a later separate `create_task` starts over at `20001`. A builder that never retries only ever needs `20001` — our previous hardcoded `20000` was simply wrong.

**Confirmed 1:1 passthroughs.** `autoBedLeveling` / `extrudeCaliFlag` / `nozzleOffsetCali` are plain ints from `auto_bed_leveling` / `auto_flow_cali` / `auto_offset_cali` (`2`/`3`/`4` came through unchanged, so they are not coerced to booleans); `bedType`, `oriModelId`, `oriProfileId`, `plateIndex` are verbatim; `amsDetailMapping` and `nozzleInfos` are the caller's JSON. The server wants `amsDetailMapping[].ams` as a **number** — a string sentinel there returns `400 type mismatch for field "amsDetailMapping[0].ams"`.

**Fed from outside this ABI.** `cover` stayed `""` and `filamentSettingIds` stayed `[]` even with every `PrintParams` string filled with sentinels, so neither is reachable through the print ABI. `preset_name`, `comments`, `extra_options`, `dev_name`, `try_emmc_print`, `print_type` and `connection_type` produce no key at all (`print_type="OBNPRINTTYPE"` and `connection_type="OBNCONNTYPE"` on `local_print_with_record` left the `/my/task` body unchanged and never appeared in any other HTTPS request).

**`plateIndex` is verbatim.** A probe with `plate_index=2` produced `"plateIndex":2` (HTTP 200); the LAN `param` on the same ABI is `Metadata/plate_2.gcode`.

**One deviation we keep.** Stock re-serializes nested objects through a JSON object, normalising their key order (`{"id":0,"type":…,"diameter":…}` came back as `{"diameter":…,"flow_type":…,"id":0,"type":…}`); but key order is meaningless to the endpoint. Top-level order does match: stock emits the body alphabetically, so `designId` sits between `cover` and `deviceId`, and `nozzleMapping` between `nozzleInfos` and `nozzleOffsetCali`. A fully populated body is pinned byte-for-byte in `tests/cloud_print_test.cpp`.
