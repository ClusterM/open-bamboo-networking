...
| `md5` | string | literal `"from_sd_card"` | Stock `02.05.00`–`02.08.02` always emit this literal — on `sdcard_print` **and** on FTPS `local_print`, even when `ftp_file_md5` is filled with a hex sentinel. A real file hash never appeared in any captured LAN `project_file`. |
