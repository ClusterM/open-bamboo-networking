...
Two safe dry runs: `--action sdcard_print` with `dst_file` naming a file that isn't on the SD card publishes the full `project_file` and uploads nothing, and reusing a `sequence_id` across runs makes firmware reject with `mqtt message verify failed` — in both cases the plugin's serialization is already on the wire, which is all the probe needs.
...
**Fed from outside this ABI.** `cover` stayed `""` and `filamentSettingIds` stayed `[]` even with every `PrintParams` string filled with sentinels, so neither is reachable through the print ABI. `preset_name`, `comments`, `extra_options`, `dev_name` and `try_emmc_print` produce no key at all.

**One deviation we keep.** Stock re-serializes nested objects through a JSON object, normalising their key order (`{"id":0,"type":…,"diameter":…}` came back as `{"diameter":…,"flow_type":…,"id":0,"type":…}`); we forward the caller's string untouched. Key order is meaningless to the endpoint. Top-level order does match: stock emits the body alphabetically, so `designId` sits between `cover` and `deviceId`, and `nozzleMapping` between `nozzleInfos` and `nozzleOffsetCali`. A fully populated body is pinned byte-for-byte in `tests/cloud_print_test.cpp`.
