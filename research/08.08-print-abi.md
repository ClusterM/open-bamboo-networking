...
2. Publish MQTT `project_file` with `"0"` cloud id placeholders and the chosen URL scheme — [payload](#project_file-payload). `sequence_id`: [§6.2](06.02-mqtt.md#mqtt-sequence_id).
...
**Plugin behaviour.** Publish MQTT `project_file` with `url=file:///<dst_file>` (and the usual plate/AMS flags) on the active session — [payload](#project_file-payload). No FTPS/S3 body upload for the model.
