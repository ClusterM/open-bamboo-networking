...
// accepts only a couple of concurrent LAN sessions, so a session left behind
// blocks the next connect until that timer expires. A clean shutdown sends
// DISCONNECT (see Agent::shutdown_lan_session) and frees the slot at once, so
// this only covers hard exits — crash, kill, PC sleep — where the DISCONNECT
// never goes out. At the old 60s that was up to ~90s of "connection failed,
// works on the second try" (#34); 20s brings it down to ~30s. The cost is one
// 2-byte PINGREQ every 20s, and faster detection of a dropped link is a bonus.
