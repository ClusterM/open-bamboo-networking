...
| "Send to Printer" dialog (`ft_*`)          | ✅ (tested on P2S) | Native    | `ft_*` over TLS :6000 (BambuTunnelLocal, `OBN_FT_TUNNEL_LOCAL` ON by default); FTPS :990 fallback. Stock uses the same :6000 wire. |
...
| `--disable-ftps-fastpath` | enabled                                                        | Disable both `OBN_FT_TUNNEL_LOCAL` and `OBN_FT_FTPS_FALLBACK`; Studio falls back to internal FTP send. See [STATUS.md § 6.14](STATUS.md#614-file-transfer-abi-ft_). |
