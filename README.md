...
| `ft_*` tunnel local (`OBN_FT_TUNNEL_LOCAL`, default ON) | Port-6000 BambuTunnelLocal for Send-to-Printer, eMMC preflight, and LAN print upload (stock path). | Native :6000 wire only (`tunnel_upload.cpp` + `abi_ft.cpp`). |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Shared with `start_local_print` / `_with_record` when `try_emmc_print`. Legacy printers without `:6000` use Studio `SendJob` → FTPS. See [STATUS.md § 6.14](STATUS.md#614-file-transfer-abi-ft_).                                                                                         |
